## 🌟 AYUSH — Data Science | AI | Machine Learning | Analytics
📍 Data Science Enthusiast | 📊 Business Analytics | 🤖 Machine Learning | 🧠 NLP | 📈 Data Visualization
- Driven and detail-oriented Data Science & Machine Learning student with hands-on experience in Python, data analytics, visualization, and end-to-end ML projects.
- Passionate about extracting insights from data and building intelligent systems that create real-world impact.

I actively work on projects focusing on NLP, sentiment analysis, web scraping, automation, data visualization, and ML model development.
Currently sharpening skills through internships, real datasets, and continuous learning.

# 🚀 About Me :-
- 🎓 BBA Student + Master's-level training in Data Science & Analytics with AI
- 🤖 Passionate about Machine Learning, NLP, Data Engineering, Visualization
- 🧑‍💻 Experienced in Python, SQL, Power BI, Tableau
- 📊 Built 20+ real-world projects (ML, EDA, NLP, Web Scraping, APIs)
- 🏆 Completed 20+ industry certifications from IBM, AWS, Deloitte, BCG, TATA, Siemens, Quantium
- 📂 Currently interning as a Data Analyst, next: ML Intern + Python Developer Intern
- 🔥 Building strong GitHub portfolio with real datasets & industry-style projects

# 🌱 Currently improving:
- Deep Learning
- Advanced Machine Learning
- Data Engineering Fundamentals

🤝 Open to internships, freelancing, collaborations and open-source work

# 🧰 Skills & Tools

🐍 Programming & Scripting
- Python • SQL • NumPy • Pandas • Matplotlib • Seaborn

🤖 Machine Learning & AI
- Scikit-learn • NLP • Sentiment Analysis • SVM • Random Forest • Logistic Regression

📊 Data Visualization
- Power BI • Tableau • Matplotlib • Seaborn

🧼 Data Engineering Basics
- Web Scraping (BeautifulSoup, APIs) • Automation • ETL

🔧 Other Tools
- Git • GitHub • Jupyter Notebook • VS Code

# 🎓 Internships
✅ CodeAlpha — Data Analyst Intern (1 Month - Present)
- Working on data analytics projects involving EDA, visualization, and reporting.


# 🏅 Certifications
-Completed 15+ industry-grade certifications from:
From global companies & platforms:

- IBM • Machine Learning
- IBM • Python for Data Science
- IBM • Data Analysis with Python
- AWS Academy • Cloud Fundamentals
- Deloitte • Data Analytics
- BCG • GenAI & Data Science
- TATA • Data Analytics with GenAI
- Siemens • Commercial Project Management
- Quantium • Data Analytics
- British Airways • Data Science
- I.T. Vedant • Master’s in Data Science & AI

# 📂 Featured Projects
🔹 1. *AI Sentiment Analysis – ML + NLP Advanced Project*
- Advanced NLP pipeline with TF-IDF, SVM tuning, and ML model comparison.
- 📌 NLP | ML | Python | Text Processing
- ➡ Repo: (https://github.com/ayush13-0/AI-Sentiment-Analysis-ML-NLP)

🔹 2. *Hybrid Data Extraction: Web Scraping + REST API*
- Combined scraping from BooksToScrape with OpenFoodFacts API → analytics & visualization.
- 📌 ETL | API | Web Scraping | Data Analysis
- ➡ Repo: (https://github.com/ayush13-0/CodeAlpha_Hybrid-Data-Extraction-Analytics)

🔹 3. *Global Retail Sales EDA*
- Data-driven insights into sales, trends, customer behavior & profitability.
- 📌 EDA | Visualization | Business Analytics
- ➡ Repo: (https://github.com/ayush13-0/CodeAlpha_Global-Retail-Sales-EDA)

🔹 4. *ML Health Insurance Claim Fraud Detection (ML)*
- Logistic Regression, Random Forest, XGBoost → ML pipeline for fraud prediction.
- 📌 ML | Fraud Analytics | Feature Engineering
- ➡ Repo: (https://github.com/ayush13-0/ML-Health-Insurance-Claim-Fraud-Detection)

🔹 5. *Consumer Sentiment Analysis*
- NLP analysis using VADER, TextBlob & ML classification models.
- 📌 NLP | Text Analytics | ML
- ➡ Repo: (https://github.com/ayush13-0/CodeAlpha_Consumer-Sentiment-Analysis)

🔹 6. *E-Commerce Python + SQL Project*
- Database queries + Python analysis → customers, orders, products & revenue.
- 📌 SQL | Python | Data Engineering
- ➡ Repo: (https://github.com/ayush13-0/Ecommerce-Python-SQL-Project)

------------------------------------------------------------------------------------------------------------------------------


# 📈 GitHub Stats
![Ayush's GitHub Stats](https://github-readme-stats.vercel.app/api?username=ayush13-0&show_icons=true&theme=tokyonight)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=ayush13-0&layout=compact&theme=tokyonight)


# AYUSH
🔗 Connect With Me
📧 Email: bhanuseenu914@gmail.com
- 🔗 LinkedIn: https://linkedin.com/in/ayush130
- 🔗 GitHub: https://github.com/ayush13-0
